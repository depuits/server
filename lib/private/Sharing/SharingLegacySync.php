...
final class SharingLegacySync implements IEventListener {
	...
	private function syncLegacySharesToShare(string $id, array $legacyMappings): void {
		...
		$allowDownload = $this->isPublicLegacyShare($legacyShares[0])
			? !$legacyShares[0]->getHideDownload()
			: $legacyShares[0]->getAttributes()?->getAttribute('permissions', 'download') !== false;
	...
	private function syncShareToLegacyShares(Share $share): void {
		...
		foreach ($share->recipients as $recipient) {
			...
			foreach ($share->sources as $source) {
				...
				$legacyShare->setHideDownload($this->isPublicLegacyShare($legacyShare) && !$allowDownload);

				$attributes = new ShareAttributes();
				$attributes->setAttribute('permissions', 'download', $this->isPublicLegacyShare($legacyShare) || $allowDownload);
